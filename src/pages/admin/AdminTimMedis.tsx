...
export default function AdminTimMedis() {
  const { siteData, updateSection } = useCMS();
  const [data, setData] = useState<any[]>(siteData.timMedis || []);

  const [editingId, setEditingId] = useState<any>(null);
  const [formData, setFormData] = useState<any>({});

  const handleSave = () => {
    updateSection('timMedis', data);
    toast.success('Data Berhasil Disimpan!', {
      description: 'Pengaturan tim medis telah diperbarui.'
    });
  };

  const handleAdd = () => {
    setEditingId('new');
    setFormData({ id: Date.now(), name: '', role: '', img: '' });
  };
  ...
  const handleDelete = (id: any) => {
    if (confirm('Hapus profil staf ini?')) {
  ...
  if (editingId) {
    return (
      <div className="space-y-4">
        <h3 className="text-lg font-bold">Edit Profil Tenaga Medis</h3>
        <input className="w-full border p-2 rounded" placeholder="Nama Lengkap & Gelar (Cth: Dr. Budi)" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
        <input className="w-full border p-2 rounded" placeholder="Jabatan / Peran (Cth: Dokter Gigi)" value={formData.role} onChange={e => setFormData({...formData, role: e.target.value})} />
        <ImageUpload 
          label="Foto Profil" 
          value={formData.img || ''} 
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-bold text-lg">Daftar Tenaga Medis</h3>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {data.map((item, idx) => (
          <div key={idx} className="border p-4 rounded bg-slate-50 flex gap-4">
            <div className="w-20 h-20 shrink-0 rounded-full overflow-hidden bg-slate-200">
